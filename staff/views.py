...
def loan_books(request, user_id):
	...
	user_active_records_total = Record.objects.filter(date_returned=None).count()
	if user_active_records_total > 3:
		messages.error(request, 'User has maximum number of active loans allowed')
	...
	available_instances = BookInstance2.objects.filter(status='a').order_by('id')
	...
	if request.method == 'POST':
		...
		if instances_to_be_loaned_out:
			...
			for an_instance in instances_to_be_loaned_out:
				record = Record(user_id=user.id, book_instance_id=an_instance.id, book_title=an_instance.book.title, date_taken_out=date.today())
				record.save()
